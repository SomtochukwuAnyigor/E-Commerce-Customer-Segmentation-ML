...
🛠️ The Technical Workflow
    ...
    Data Cleaning: Processed 500k+ rows, handling missing values and filtering out transaction returns to ensure data integrity.

    Feature Engineering: Built an RFM table from scratch, calculating Recency, Frequency, and Monetary value for each customer.

    Pre-processing: Applied Log Transformation to handle heavy right-skewness and StandardScaler to normalize features for distance-based clustering.

    Optimal K Discovery: Utilized the Elbow Method to identify the optimal number of segments (K=4).
